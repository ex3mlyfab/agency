...
export function DeceasedForm({
    ...
    const { data, setData, submit, processing, errors } = useForm<DeceasedFormData>({
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Card>
                ...
                <CardContent className="grid gap-6 px-6 py-6 sm:grid-cols-2">
                    ...
                    <div className="space-y-1.5">
                        ...
                        <DatePicker
                            value={data.date_of_birth ? parseISO(data.date_of_birth) : undefined}
                    ...
                    {/* Date of Death */}
                    <div className="space-y-1.5">
                        <Label htmlFor="date_of_death" className="font-semibold">
                            Date of Death <span className="text-destructive">*</span>
                        </Label>
                        <DatePicker
                            value={data.date_of_death ? parseISO(data.date_of_death) : undefined}
                            onChange={(date) => setData('date_of_death', date ? format(date, 'yyyy-MM-dd') : '')}
                            disabled={processing}
                            className={cn(errors.date_of_death && 'border-destructive')}
                        />
                    ...
                    {/* Cause of Death */}
                    <div className="space-y-1.5">
                        <Label htmlFor="cause_of_death" className="font-semibold">
                            Cause of Death
                        </Label>
                        <Input
                            id="cause_of_death"
                    ...
                    <div className="space-y-1.5">
                        ...
                        <Select
                            ...
                            onValueChange={(val) => setData('chamber_id', val === 'none' ? '' : val)}
                            disabled={processing}
                        >
                            <SelectTrigger
                                id="chamber_id"
                                className={cn(errors.chamber_id && 'border-destructive')}
                            >
                    ...
                    <div className="space-y-1.5">
                        ...
                        <DatePicker
                            value={data.stored_at ? parseISO(data.stored_at) : undefined}
                            onChange={(date) => setData('stored_at', date ? format(date, 'yyyy-MM-dd') : '')}
                            disabled={processing || !data.chamber_id}
                            className={cn(errors.stored_at && 'border-destructive')}
                        />
            ...
            <Card>
                ...
                <CardContent className="grid gap-6 px-6 py-6 sm:grid-cols-2">
                    {/* Relative Name */}
                    <div className="space-y-1.5">
                        <Label htmlFor="relative_name" className="font-semibold">
                        ...
                        <Input
                            ...
                            disabled={processing}
                            className={cn(errors.relative_name && 'border-destructive')}
                        />
                    ...
                    {/* Relative Phone */}
                    <div className="space-y-1.5">
                        <Label htmlFor="relative_phone" className="font-semibold">
                        ...
                        <Input
                            ...
                            onChange={(e) => setData('relative_phone', e.target.value)}
                            disabled={processing}
                            className={cn(errors.relative_phone && 'border-destructive')}
                        />
                    ...
                    {/* Relationship */}
                    <div className="space-y-1.5">
                        <Label htmlFor="relative_relationship" className="font-semibold">
                            Relationship <span className="text-destructive">*</span>
                        </Label>
                        <Input
                            id="relative_relationship"
                            value={data.relative_relationship}
                            onChange={(e) => setData('relative_relationship', e.target.value)}
                            placeholder="e.g. Spouse, Child, Parent"
                            disabled={processing}
                            className={cn(errors.relative_relationship && 'border-destructive')}
                        />
                    ...
                    {/* Address */}
                    <div className="space-y-1.5">
                        <Label htmlFor="relative_address" className="font-semibold">
                            Address
                        </Label>
                        <Input
                            id="relative_address"
                            value={data.relative_address}
                            onChange={(e) => setData('relative_address', e.target.value)}
                            disabled={processing}
                        />
